window.discountsByName = window.discountsByName || {};
window.discountsByName["__MEMBERSHIP_NAME__"] = "__DISCOUNT_CODE__";
...
window.showMemberBenefitsModal = async (options) => {
  ...
  await Promise.all(
    options.map(async ({ name, locks }) => {
      ...
      clone.querySelector(".status").dataset.discount = discountsByName[name];
  ...
  async function fetchAccountData() {
    ...
    document.querySelectorAll(".membership").forEach((membership) => {
      ...
      locks.map((lockAddress) => {
        ...
        const mainnetLock = new web3Mainnet.eth.Contract(lockAbi, lockAddress);
        mainnetLock.methods
        ...
        const polygonLock = new web3Polygon.eth.Contract(lockAbi, lockAddress);
        polygonLock.methods
          .getHasValidKey(selectedAccount)
          .call()
          .then((result) => {
...
function updateUnlockUIElements(unlockState) {
  // We show all .unlock-content elements
...
window.addEventListener("memberBenefits.status", function (event) {
  const lockAddresses = __LOCK_ADDRESSES__;
  const lockAddress = event.detail.lock.toString();

  // Only continue if lock address belongs to this membership
  if (lockAddresses.indexOf(lockAddress) === -1) return;
  ...
  if (window.activeDiscountCode === "__DISCOUNT_CODE__") {
...
const load_script = {
  ...
  next: function () {
    ...
    script.onload = () => {
      ...
      if (load_script.index < load_script.scripts.length - 1)
        load_script.next();
      item.resolve();
    };
  ...
  function (src) {
    if (src) {
      ...
      for (var i = 0; i < load_script.scripts.length; i++) {
        if (load_script.scripts[i].src == src)
          return load_script.scripts[i].promise;
      }
      // Add to the queue
      var item = { src: src };
...
// Load external js and add modal to DOM if necessary
if (!document.getElementById("mb-modal")) {
  ...
  ].forEach(window.load_scripts);

  document.body.insertAdjacentHTML(
    "beforeend",
    `<div class="mb-modal" id="mb-modal">
...
window.openModalButtons =
  window.openModalButtons || document.querySelectorAll("[data-modal-target]");
window.closeModalButtons =
  window.closeModalButtons || document.querySelectorAll("[data-close-button]");
